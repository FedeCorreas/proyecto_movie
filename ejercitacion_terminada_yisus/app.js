...
const fs = require('fs');
const secciones = require('./utiles');
const movies = [
	{
...
http.createServer((req, res) => {
	...
	let cantidad = movies.length; // cantidad total de peliculas

	// Antes de hacer cualquier cosa, ordenamos el array de peliculas (por título):
	movies.sort((x, y) => (x.title > y.title) ? 1 : (x.title < y.title) ? -1 : 0);

	switch (req.url) { // Route System

		// Home
		case '/':
			listadoEnHTML = secciones.ListadoEnFormatoHtml(movies, 'title', 'overview');
			res.end(`<h1>Bienvenidos a DH Movies</h1>
			<h2>El mejor sitio para encontrar las mejores películas, incluso mucho mejor que Netflix, Cuevana y PopCorn.</h2>
			<h3>Total de películas en cartelera:</h3>
			<p>${cantidad}</p>
			<h3>Listado de películas:</h3>
			<p>${listadoEnHTML}</p>
			<h3>Recordá que podés visitar las siguientes secciones:</h3>
			${secciones.footer}`);
			break;
		...
		// Más votadas
		case '/mas-votadas':
			listadoEnHTML = secciones.listarMasVotadas(movies);
			res.end(`<h1>Mas Votadas</h1>
			<h3>Total de películas:</h3>
			<p>${secciones.cantidadMasVotadas}</p>
			<h3>Listado de películas:</h3>
			<p>${listadoEnHTML}</p>
			<h3>Recordá que podés visitar las siguientes secciones:</h3>
			${secciones.footer}`);
		...
		// Sucursales
		case '/sucursales':
			let sucursalesHtml = secciones.listadoSucursalesHtml(theaters);
			res.end(`<h1>Nuestras Sucursales</h1>
			<h3>Total de salas:</h3>
			<p>${secciones.totalSalas(theaters)}</p>
		...
		case '/contacto':
			...
			<h3>¿Tenés algo para contarnos? Nos encanta escuchar a nuestros
			...
			respuestas inmediatas a los problemas más comunes.</h3>
			<h3>Recordá que podés visitar las siguientes secciones:</h3>
			${secciones.footer}`);
			break;
		...
		case '/preguntas-frecuentes':
			let faqsHtml = secciones.ListadoEnFormatoHtml(faqs, 'faq_title', 'faq_answer');
